...
export default function TokenConversion({ 
  ...
  const [conversionDirection, setConversionDirection] = useState<"strkToTarkPlay" | "tarkPlayToStrk">("strkToTarkPlay")
  ...
  const toggleConversionDirection = () => {
    setConversionDirection(prev => prev === "strkToTarkPlay" ? "tarkPlayToStrk" : "strkToTarkPlay")
    setInputAmount("")
    setError(null)
  }
  ...
  const handleConvert = async () => {
    if (!isValidInput) {
      if (numericAmount <= 0) {
        ...
        (conversionDirection === "strkToTarkPlay" && numericAmount > mockUserBalance) ||
        (conversionDirection === "tarkPlayToStrk" && numericAmount > mockTarkPlayBalance)
      ) {
    ...
    try {
      ...
      const successMessage = conversionDirection === "strkToTarkPlay"
  ...
  const getTokenInfo = (isSource: boolean) => {
    ...
    return {
      ...
      icon: isStrkToken 
        ? <Image src="/strk-svg.svg" alt="STRK Token" width={20} height={20} className="w-5 h-5" />
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="bg-gray-900 text-white rounded-xl shadow-lg overflow-hidden border border-purple-500/20">
        <div className="flex items-center justify-between p-4 border-b border-purple-500/30">
          <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-purple-600">Swap</h2>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="rounded-lg bg-gray-800 p-4 border border-purple-500/20">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-right text-xl font-medium">{isValidInput ? receivedAmount.toFixed(6) : "0.0"}</div>
            </div>
          ...
          <div className="rounded-lg bg-gray-800 p-4 space-y-2 border border-purple-500/20">
            <div className="flex items-center justify-between text-sm">
              ...
              <button className="h-6 w-6 p-0 hover:bg-purple-500/20 rounded-full transition-colors">
                <RotateCw className="rotate-[135deg] text-purple-400" size={24} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-purple-300">
                {isValidInput ? receivedAmount.toFixed(6) : "0.0"} {destinationToken.name}
              </span>
            </div>
          </div>
        ...
        <div className="p-4">
          <button
            className={`w-full py-6 text-lg font-medium rounded-lg transition-colors ${
              isValidInput ? "bg-gradient-to-r from-purple-500 to-purple-700 hover:from-purple-600 hover:to-purple-800" : "bg-gray-700 text-gray-400 cursor-not-allowed"
            }`}
